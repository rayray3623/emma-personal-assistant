...
// tool-call round trips to Gmail/Calendar/Supabase) runs afterwards with no
// risk of hitting Twilio's ~15s webhook timeout. This mirrors the pattern
// already used for Cara's WhatsApp handler and Q's background test runner.
//
// PRIVATE, SINGLE-USER LINE: only messages from Ray's own WhatsApp number
// (EMMA_OWNER_WHATSAPP_NUMBER) are ever processed or replied to. Anything
// else is silently dropped — there is no one else this line is for.
//
...
across both the web chat and WhatsApp — a conversation started on one continues seamlessly on the
...
const TOOLS = [
  ...
  {
    name: 'send_image',
    description: "Send an image to Ray over WhatsApp, alongside your text reply. image_url must be a public, directly-fetchable URL (e.g. one found via web search) — you can't attach an image from Ray's Gmail directly, since Gmail attachment URLs require authentication Twilio can't provide. Use this to actually show him something rather than just describing it, when a genuine image is available.",
    input_schema: {
      type: 'object',
      properties: {
...
async function fetchPendingProactiveNotes() {
  ...
  return `\n\n---\n\nTHINGS TO MENTION\n\nSince you last spoke, the following came up and were judged worth telling Ray about (this may already have been sent to him as a proactive WhatsApp message — don't repeat it verbatim, just hold it as context).\n\n${lines.join('\n')}`;
}
...
// --- Twilio-specific pieces ---

function parseFormBody(rawBody, isBase64) {
  const decoded = isBase64 ? Buffer.from(rawBody, 'base64').toString('utf-8') : rawBody;
...
// Validates that the request genuinely came from Twilio, per their signed-
// request scheme: HMAC-SHA1 of (full URL + sorted POST params concatenated
// as key+value pairs), base64-encoded, compared against X-Twilio-Signature.
...
// Twilio only supports these image types cleanly for us to hand to Claude
// (Claude's API accepts jpeg/png/gif/webp); anything else (video, audio,
// pdf, vcard) we acknowledge but don't try to process as an image.
const SUPPORTED_IMAGE_TYPES = new Set(['image/jpeg', 'image/png', 'image/gif', 'image/webp']);

// Twilio-hosted media URLs require the same Basic Auth as the REST API —
// they are not publicly fetchable without credentials.
async function fetchTwilioMedia(mediaUrl) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch media (${res.status}) from ${mediaUrl}`);
...
// WhatsApp voice notes arrive as audio/ogg (opus codec) almost always;
// covering mp4/mpeg/amr too in case a client ever sends something else.
const AUDIO_TYPES_RE = /^audio\//;

// Transcribes a voice note via OpenAI's Whisper API. Kept separate from
// fetchTwilioMedia (which just downloads bytes) so the two concerns —
// "get the file from Twilio" and "turn it into text" — stay independent;
// fetchTwilioMedia is reused unchanged for audio, same as it already is
// for images.
...
// failure. Every other external call in this file is wrapped defensively
// (see fetchCurrentState, findOverlappingEvents callers, transcribeAudio,
// etc.) so one hiccup degrades gracefully instead of taking down the whole
// turn — this call was the one exception, and an uncaught network error or
// malformed response here was exactly what produced the generic "something
// went wrong" fallback, with no indication of what actually failed.
// Retrying once catches the common transient case (dropped connection,
// brief timeout) before giving up.
...
exports.handler = async function (event) {
  // Background functions: Netlify has already returned 202 to Twilio by
  ...
  try {
    ...
    const params = parseFormBody(event.body, event.isBase64Encoded);
    const from = (params.From || '').replace(/^whatsapp:/, '');
    const incomingText = params.Body || '';
    const numMedia = parseInt(params.NumMedia || '0', 10) || 0;

    const authToken = process.env.TWILIO_AUTH_TOKEN;
    const signature = event.headers['x-twilio-signature'] || event.headers['X-Twilio-Signature'];
    // Reconstruct the exact URL Twilio signed. Background functions run
    // behind the same host Twilio actually posted to.
    const proto = event.headers['x-forwarded-proto'] || 'https';
    const host = event.headers['host'];
    const fullUrl = `${proto}://${host}${event.path}`;
    if (!validateTwilioSignature(fullUrl, params, signature, authToken)) {
      console.error('Twilio signature validation failed — dropping request.');
      return;
    }
    ...
    // media).
    if (!incomingText.trim() && numMedia === 0) return;
    ...
    for (let i = 0; i < numMedia; i++) {
      ...
      const contentTypeHint = params[`MediaContentType${i}`];
      if (!mediaUrl) continue;

      if (AUDIO_TYPES_RE.test(contentTypeHint || '')) {
        try {
          const { base64, contentType } = await fetchTwilioMedia(mediaUrl);
          const transcript = await transcribeAudio(base64, contentType || contentTypeHint);
      ...
      if (!SUPPORTED_IMAGE_TYPES.has(contentTypeHint)) {
        unsupportedAttachments.push(contentTypeHint || 'unknown file type');
        continue;
      }
      try {
        const { base64, contentType } = await fetchTwilioMedia(mediaUrl);
        imageBlocks.push({
          type: 'image',
          source: { type: 'base64', media_type: contentType || contentTypeHint, data: base64 },
        });
      } catch (err) {
        console.error('Failed to fetch an image attachment:', err.message);
    ...
    // WhatsApp has no client-side history array like the web chat — the
    ...
    // The reply itself is fully computed at this point — don't let a
    // transient Twilio send failure here fall through to the outer catch,
    // which would just attempt an equally-doomed second send of a
